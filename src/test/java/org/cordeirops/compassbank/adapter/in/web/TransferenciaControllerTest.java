...
import org.cordeirops.compassbank.adapter.in.web.dto.TransferenciaRequestDTO;
...
class TransferenciaControllerTest {
    ...
    void transferir_dadosValidos_retorna201() throws Exception {
        TransferenciaResultadoDTO resultado = new TransferenciaResultadoDTO(
        ...
        mockMvc.perform(post("/api/v1/transferencias")
                        ...
                        .content(objectMapper.writeValueAsString(
                                new TransferenciaRequestDTO(ORIGEM_ID, DESTINO_ID, new BigDecimal("200.00"))
                        )))
    ...
    void transferir_saldoInsuficiente_retorna422() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/transferencias")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new TransferenciaRequestDTO(ORIGEM_ID, DESTINO_ID, new BigDecimal("500.00"))
                        )))
    ...
    void transferir_contaNaoEncontrada_retorna404() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/transferencias")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new TransferenciaRequestDTO(ORIGEM_ID, DESTINO_ID, new BigDecimal("100.00"))
                        )))
    ...
    void transferir_mesmaConta_retorna400() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/transferencias")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new TransferenciaRequestDTO(ORIGEM_ID, ORIGEM_ID, new BigDecimal("100.00"))
                        )))
    ...
    void transferir_valorZero_retorna400PorValidacao() throws Exception {
        mockMvc.perform(post("/api/v1/transferencias")
                        .contentType(APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new TransferenciaRequestDTO(ORIGEM_ID, DESTINO_ID, BigDecimal.ZERO)
                        )))
                .andExpect(status().isBadRequest());
    }
